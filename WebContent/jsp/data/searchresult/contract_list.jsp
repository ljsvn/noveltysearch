...
<% String local = request.getContextPath(); %>
{
    ...
    , '-',
		{
	      text: '打印', iconCls: 'x-icon-printer', tooltip: '打印选中的合同', handler: function(btn)
	      {
	         var grid = btn.findParentByType('ods.grid');
			if (typeof (grid.getSelectionModel().getSelected()) == 'undefined') {
				Ext.example.msg("系统提示", "<b>" + "请勾选多选框内容" + "</b>");
				...
				if (records.length > 1) {
	               ...
	               var contractPk = records[0].get('contractPk');
	               window.open('<%=local %>/jsp/print/contract_print.jsp?contractPk='+contractPk, 'mywindow', ' menubar=no, toolbar=no, scrollbars=yes');
	         }}  
	      }
        }
	]
