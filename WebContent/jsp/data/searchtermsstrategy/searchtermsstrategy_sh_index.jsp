...
<% String local = request.getContextPath(); %>
{
	...
	,
  {
    ...
    ,
    {
      header: '完成时间', sortable: true, dataIndex: "endTime"
    }
    ]
    ,listeners:{
      ...
      text: '修改', tooltip: '修改选中的合同管理记录', iconCls: 'x-icon-edit', handler: function(btn)
      {
        ...
        else
        {
          ...
          else
          {
            ...
            win=new Ext.Window(
            {
              layout: 'border', title: '修改选中的合同管理记录--网页对话框', store: grid.store, autoDestroy: true, width: 900, height: 350, plain: true, modal: true, iconCls: 'bogus', resizable: false, closable: true, items: [
              {
                layout: 'fit', region: 'center', xtype: 'panel', plugins: [new Ext.ux.Plugin.RemoteComponent(
                {
                  url: '<%=local %>/search/terms/strategy/modify_sh_page.ods', method: 'post', params:
                  {
                    'contractPk': contractPk
                  }
                }
