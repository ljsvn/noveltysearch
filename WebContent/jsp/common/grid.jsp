...
<%
	...
	String total = "0";
	if(qr != null) {
	    total = String.valueOf(qr.getQueryPageJavaBean().getTotalCount());
	}
	...
	for(int i = 0; qr != null && i < qr.size(); i++) {
	    ...
	    Object obj = qr.get(i);
	    String item = "";
	    for(int arrayIndex = 0; fieldList != null && arrayIndex < fieldList.length; arrayIndex++) {
	        try {
	            Field field = obj.getClass().getDeclaredField(fieldList[arrayIndex]);
	            if(field != null) {
	                builder.append(item).append(fieldList[arrayIndex]).append(":");
	                String methodName = new StringBuilder("get").append(StringUtil.upperFirst(fieldList[arrayIndex])).toString();
	                Method method = obj.getClass().getMethod(methodName);
	                String value = String.valueOf(method.invoke(obj, new Object[] {}));
	                if(StringUtil.empty(value) || StringUtil.equals("null", value.toLowerCase())) {
	                    value = "";
	                } 
	                value = value.replaceAll(File.separator + "r", "");
	                value = value.replaceAll(File.separator + "r" + File.separator + "n", "");
