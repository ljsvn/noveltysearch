...
<% String local = request.getContextPath(); %>
{
	...
	items:[
	{
		region : 'center', 
		xtype : 'ods.grid', 
		id: 'bulletinreceive_whole_grid_id', 
		border: false,
		...
		store: new Ext.data.Store(
		{
			...
			listeners :{
				'load' : function(store ,records,options ) {
					var grid = Ext.getCmp('bulletinreceive_whole_grid_id');
			...
			,[
                ...
                "receivePk", "orgPk", "userPk", "userName", "bulletinIsread", "receiveOrgPk", "receiveUserPk", "receiveDeleteType",
                "receiveCreateTime","createUserName"
            ])
            ,baseParams : {sendType : 0,isread: '<%=request.getParameter("isread") %>'}
		}
		), 
		columns : [
			...
			,{header : '发送人',	sortable:true, width: 120,	dataIndex : 'createUserName'}
		...
		, '-',
		{
			...
			disabled: true, isdel : true, iconCls: 'x-icon-delete', handler: function(btn){
		        ...
		        if (Ext.Msg.confirm("系统提示",'确定删除公告标题为：<br>' + records[0].get('bulletinTitle') + '<br>的信息吗？', 
			        function(v){if (v == "yes"){
			            Ext.Ajax.request({
		              		url: '<%=local %>/bulletin/receive/delete_info.ods', 
		               		params:{
								'receivePk' : records[0].get('receivePk')
		              		},
