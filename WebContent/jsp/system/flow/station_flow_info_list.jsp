...
<% String local = request.getContextPath(); %>
{
	...
	items: [{
		...
		region: 'center',
		xtype: 'ods.grid',
		id: 'station_flow_info_list_grid',
		pagesize: 20,
		...
		store: new Ext.data.Store({
			...
			listeners :{
				'load' : function(store ,records,options ) {
					var grid = Ext.getCmp('station_flow_info_list_grid');
					var orgPk = store.baseParams['orgPk'];
					Ext.each(grid.toolbars[0].items.items, function(bar){
						if(orgPk){
							if(bar.isadd){
								bar.enable();
